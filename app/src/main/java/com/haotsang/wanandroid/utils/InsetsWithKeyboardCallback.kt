...
class InsetsWithKeyboardCallback(window: Window, private val onAction: (height: Int, ime: Boolean) -> Unit) : OnApplyWindowInsetsListener, WindowInsetsAnimationCompat.Callback(
    ...
    init {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.Q) {
            ...
            window.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_MASK_STATE)//SOFT_INPUT_STATE_UNSPECIFIED
        }
    }
    ...
    override fun onProgress(insets: WindowInsetsCompat, runningAnimations: MutableList<WindowInsetsAnimationCompat>): WindowInsetsCompat {
        val typesInset = lastWindowInsets!!.getInsets(WindowInsetsCompat.Type.ime())
        val otherInset = lastWindowInsets!!.getInsets(WindowInsetsCompat.Type.systemBars())
        val subtract = androidx.core.graphics.Insets.subtract(typesInset, otherInset)
        val diff =
            androidx.core.graphics.Insets.max(subtract, androidx.core.graphics.Insets.NONE)

        val showIme = lastWindowInsets!!.isVisible(WindowInsetsCompat.Type.ime()) && typesInset.bottom > 0
        onAction.invoke(diff.bottom, showIme)
        return insets
    }
